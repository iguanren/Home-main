...
```
├── index.html              # 主页（Hero 读书卡 + 壁纸卡片流 + 灯箱 + 弹窗）
├── style.css               # 全手写样式，无任何前端框架依赖
├── data.json               # 壁纸数据（Actions 每日自动更新，滚动保留 30 天）
├── favicon.svg
...
2. 脚本抓必应官方接口最近 8 天壁纸（idx=0 今天 ~ idx=7），合并进 `data.json`，按日期去重、滚动保留最近 30 天
3. 数据有变化才提交推送（无空提交）
4. **EdgeOne Pages** 连接仓库，检测到 push 自动重新部署
5. 用户访问 iguanren.eu.org 走 EdgeOne 边缘节点，图片直连必应 CDN（缩略图 400x240 / 大图 4K UHD）

> 注：必应官方接口仅开放最近 8 天数据，因此卡片流**上线时 8 张，每天自动 +1，30 天滚动满窗**。
